...
for i = 1:m
a1 = X(i,:);
h = sigmoid( [1 sigmoid(a1 * Theta1')] * Theta2' );
yk = zeros(1,K);
yk(y(i)) = 1;
J = J + sum( -1 * log(h)* yk' - log(1 - h)*(1-yk'));
end;
J = 1 / m * J;
...
delta_accum1 = zeros(size(Theta1));
delta_accum2 = zeros(size(Theta2));
for t = 1:m
     a_1 = X(t,:);  
	z_2 = a_1 * Theta1';
	a_2 = [1 sigmoid(z_2)];
...
delta_accum1 = delta_accum1 + delta_2(2:end)' * a_1;
delta_accum2 = delta_accum2 + delta_3' * a_2;
